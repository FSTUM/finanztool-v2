{% autoescape off %}
    ...
    \begin{textblock}{1}(0,196) \rule[0.5ex]{2mm}{1pt} \end{textblock}
    ...
    \raggedright
    \notblank{ {{ rechnung.kunde.organisation | latexescape }} }
        {\parbox[c]{8cm}{ {{ rechnung.kunde.organisation | latexescape }} }\\[1ex]}{}
    \notblank{ {{ rechnung.kunde.suborganisation | latexescape }} }
        {\parbox[c]{8cm}{ {{ rechnung.kunde.suborganisation | latexescape }} }\\[1ex]}{}
    \notblank{ {{ rechnung.kunde.vorname | latexescape }}{{ rechnung.kunde.name | latexescape }} }
        {\parbox[c]{8cm}{ {{ rechnung.kunde.vorname | latexescape }} {{ rechnung.kunde.name | latexescape }}  }\\[1ex]}{}
    {\parbox[c]{8cm}{ {{ rechnung.kunde.strasse | latexescape }} }\\[1ex]}
    \ifnum\pdfstrcmp{ {{ rechnung.kunde.land | latexescape }} }{ Deutschland }=0
    ...
    \hfill{\parbox[c]{7.5cm}{
        \begin{tabular}{ll}
            Kundennummer: & {% localize off %}{{ rechnung.kunde.knr }}{% endlocalize %} \\[-0.5ex]
    ...
    \normalsize
    \ifnum\pdfstrcmp{ {{ rechnung.kunde.anrede | latexescape }} }{ w }=0
        Sehr geehrte Frau {{ rechnung.kunde.name | latexescape }},\\[2ex]
    \else
        \ifnum\pdfstrcmp{ {{ rechnung.kunde.anrede | latexescape }} }{ m }=0
            Sehr geehrter Herr {{ rechnung.kunde.name | latexescape }},\\[2ex]
        \else
    ...
    {% else %}
        ...
        {% endif %}%
            ...
            {\bf RE {{ rechnung.rnr_string }}
            \ KD {% localize off %}{{ rechnung.kunde.knr }}{% endlocalize %}}.\\
